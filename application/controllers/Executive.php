...
class Executive extends CI_Controller {
    ...
    public function __construct() {
        ...
        if (!isset($_SESSION['executive_id']) && !isset($_SESSION['ex_staff_type'])) {
...
//        $this->load->view('admin/layout', $data);
     ...
     public function change_theme($val = '') {
        ...
        $this->db->where('staff_id', $_SESSION['executive_id']);
        $this->db->update('mst_staff', array('theme_type' => $val));
        echo 0;
    }

    public function signOut() {
        $this->session->unset_userdata('executive_id');
        $this->session->unset_userdata('ex_staff_type');
        $this->session->unset_userdata('ex_staff_username');
...
//       print("<pre>".print_r( $data['leads'],true)."</pre>"); die;
    ...
    public function manage_product() {
        ...
        $config['total_rows'] = $this->ccm->get_all_count('mst_product', 'p_id', '', 'p_id');
        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['reuse_query_string'] = TRUE;
        $config['full_tag_open'] = '<ul class="pagination pagination-split justify-content-end">';
        ...
        $data['Products'] = $this->ccm->get_all_data('mst_product', 10, 'p_id', '', 'p_id');
//       print("<pre>".print_r( $data['leads'],true)."</pre>"); die;
...
//                    }
    ...
    public function product_form($id = '') {
        ...
        $data['tellus'] = $this->ccm->get_all_selected_by_condition2('tellus_qustion', array('active' => 1));
...
public function manage_sell_order() {
    ...
    if($_SESSION['ex_staff_type']==1){
        $data['order'] = $this->ccm->get_all_data_sell_order($_SESSION['executive_id']);
    }else{
        $data['order'] = $this->ccm->get_all_data_sell_order();
    }
   
//       print("<pre>".print_r( $data['leads'],true)."</pre>"); die;
    $data['viewfile'] = "executive/manage_sell_order/manage_sell_order";
    $this->load->view('executive/layout', $data);
}
...
public function order_form($id = ''){
    ...
    $data['viewfile'] = 'executive/manage_sell_order/order_form';
    $this->load->view('executive/layout', $data);
}
  ...
  public function submit_order() {
    if ($_POST) {
        ...
        $data = array(
            ...
            'updated_by' => $_SESSION['executive_id'],
...
public function tellus_form($id = '') {
  ...
  public function manage_review() {
    ...
    $this->load->library('pagination');
    $config['base_url'] = base_url() . 'executive/manage_review';
...
//       print("<pre>".print_r( $data['leads'],true)."</pre>"); die;
    $data['viewfile'] = "executive/manage_review/manage_review";
    $this->load->view('executive/layout', $data);
}
...
public function review_form($id = ''){
    ...
    $data['viewfile'] = 'executive/manage_review/review_form';
    $this->load->view('executive/layout', $data);
}

//Screen Condition
public function manage_screen_cond() {

    $this->load->library('pagination');
    $config['base_url'] = base_url() . 'admin/manage_screen_cond';
    $config['total_rows'] = $this->ccm->get_all_count('screen_condition_qsn', 'qsn_name', '', 'qsn_name');
    $config['per_page'] = 10;
    $config['page_query_string'] = TRUE;
    $config['reuse_query_string'] = TRUE;
    $config['full_tag_open'] = '<ul class="pagination pagination-split justify-content-end">';
    ...
    $data['title'] = 'List Questions';
    $data['Questions'] = $this->ccm->get_all_data('screen_condition_qsn', 10, 'qsn_name', '', 'qsn_name');
//       print("<pre>".print_r( $data['leads'],true)."</pre>"); die;
...
public function edit_device_report($id = ''){
    ...
    $data['viewfile'] = 'executive/manage_device_report/device_report_question';
    $this->load->view('executive/layout', $data);
}
